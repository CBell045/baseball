...
# Set page config
st.set_page_config(
    page_title="Baseball Player Analysis Dashboard", page_icon="⚾", layout="wide"
)
...
def load_data():
    try:
        ...
        st.error(
            "Error: player_analysis.csv not found. Please run multi_regression.py first."
        )
        return None
...
if df is not None:
    ...
    allplayers_df = pd.read_parquet("parquets/allplayers.parquet").drop_duplicates(
        subset=["id"]
    )
    filtered_df = filtered_df.merge(
        allplayers_df[["id", "first", "last"]], on="id", how="left"
    )
    filtered_df["name"] = filtered_df["first"] + " " + filtered_df["last"]
    filtered_df.drop(columns=["first", "last"], inplace=True)
    filtered_df = filtered_df[
        ["name"] + [col for col in filtered_df.columns if col != "name"]
    ]
    ...
    with col3:
        st.metric("Number of Players", len(filtered_df))

    # Create tabs for different visualizations
    tab1, tab2, tab3 = st.tabs(
        ["Performance Comparison", "Prediction Analysis", "Player Details"]
    )
    ...
    with tab1:
        ...
        fig.update_yaxes(range=[0.1, 0.35])
    ...
    improvement_pct = (
        (filtered_df["combined_prediction"] - filtered_df["current_batting_avg"])
        / filtered_df["current_batting_avg"]
        * 100
    )
    ...
    with col1:
        ...
        for _, player in top_improvers.iterrows():
            st.write(
                f"Player {player['name']}: {player['current_batting_avg']:.3f} → {player['combined_prediction']:.3f}"
            )
